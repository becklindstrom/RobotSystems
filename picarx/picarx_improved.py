...
class Picarx(object):
    ...
    def __init__(self, 
        ...
        # --------- config_flie ---------
        if on_the_robot:
            self.config_flie = fileDB(config, 777, os.getlogin(), on_the_robot=True)
        else:
            self.config_flie = fileDB(config, 777, on_the_robot=on_the_robot)
    ...
    def backward(self, speed):
        ...
        if current_angle != 0:
            ...
            power_scale = 1 #(100 - abs_current_angle) / 100.0 
    ...
    def forward(self, speed):
        ...
        if current_angle != 0:
            ...
            radius = wheel_base/math.atan(abs_current_angle)
            vf_out = ((radius+(trackwidth/2))/radius)
            vf_in = ((radius-(trackwidth/2))/radius)
            ...
            if (current_angle / abs_current_angle) > 0:
                self.set_motor_speed(1, 1*speed * vf_out)
                self.set_motor_speed(2, -speed * vf_in) 
            else:
                self.set_motor_speed(1, speed * vf_in)
                self.set_motor_speed(2, -1*speed * vf_out)
        else:
            self.set_motor_speed(1, speed)
